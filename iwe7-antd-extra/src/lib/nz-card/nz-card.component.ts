...
export class NzCardComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  gridList: QueryList<NzCardGridDirective>;

  @ContentChildren(NzCardActionComponent)
  actionList: QueryList<NzCardActionComponent>;
  ...
  getCompatibleHoverable() {
    const status =
      typeof this.nzHoverable !== "undefined" && this.nzHoverable !== false;
  ...
  _setClassMap() {
    ...
    this._classList = [
      ...
      typeof this.nzBordered !== "undefined" &&
        this.nzBordered !== false &&
        `${this._prefixCls}-bordered`,
      this.getCompatibleHoverable() && `${this._prefixCls}-hoverable`,
      typeof this.nzNoPadding !== "undefined" &&
        this.nzNoPadding !== false &&
        `${this._prefixCls}-no-padding`,
      // padding-transition
      // transition
      this.gridList &&
        this.gridList.length &&
        `${this._prefixCls}-contain-grid`,
  ...
  _tabChange(value: any) {
    this.body = this.tabList.find(
      (item, idx) => idx === value.index
    ).contentTpl;
