  ...
  if (ts.isDecorator(node)) {
    ...
    if (callExpression && ts.isCallExpression(callExpression)) {
      ...
      if (ts.isIdentifier(decoratorExpression)) {
        ...
        const ngCoreImports = resolveImportSymbolsFromModule(
          node,
          "@angular/core"
        );
        ...
        return (
          namespacePropertyName === "Component" &&
          namespaceName === ngCoreImports.__namespace
        );
      }
...
export const isStyleUrls = (node: ts.Node): node is ts.PropertyAssignment =>
  isPropertyAssignmentFor(node, "styleUrls");

export const isImportFromModule = (
  node: ts.Node,
...
export const resolveImportSymbolsFromModule = (
  node: ts.Node,
  moduleIdentifier: string
) => {
  const importSymbols: {
  ...
  return node
    ...
    .statements.filter(statement =>
      isImportFromModule(statement, moduleIdentifier)
    )
    .map(
    ...
    .reduce((symbols, importClause) => {
      ...
      if (ts.isNamedImports(importNode)) {
        ...
        for (let specifier of importSpecifier) {
          ...
          const importedFrom = specifier.propertyName
