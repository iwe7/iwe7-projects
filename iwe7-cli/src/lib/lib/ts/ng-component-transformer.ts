...
export const transformComponentSourceFiles: ComponentSourceFileTransformer = ({
  template,
  stylesheet
}) =>
  transformComponent({
    templateUrl: node => {
      ...
      const templatePath = node.initializer
        .getText()
        .substring(1, node.initializer.getText().length - 1);
      ...
      if (typeof inlinedTemplate === "string") {
        ...
        const synthesizedSourceText = "template: `"
          .concat(inlinedTemplate)
          .concat("`");
