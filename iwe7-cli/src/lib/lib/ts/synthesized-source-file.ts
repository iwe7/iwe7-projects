...
export function isSynthesizedSourceFile(
  sourceFile: ts.SourceFile
): sourceFile is SynthesizedSourceFile {
 ...
 */
export function replaceWithSynthesizedSourceText(
  node: ts.Node,
  replacement: string
 ...
 */
export function writeSourceText(
  sourceFile: ts.SourceFile | SynthesizedSourceFile
): string {
  ...
  if (isSynthesizedSourceFile(sourceFile)) {
    ...
    for (let replacement of sourceFile.__replacements) {
      newSource = newSource
        .concat(originalSource.substring(position, replacement.from))
