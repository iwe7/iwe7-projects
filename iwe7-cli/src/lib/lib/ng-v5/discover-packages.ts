...
console.log(NgPackageConfig);
/** Creates a SchemaClass for `NgPackageConfig` */
const NgPackageSchemaClass = SchemaClassFactory<NgPackageConfig>(
  ngPackageSchemaJson
);

/** Instantiates a concrete schema from `NgPackageConfig` */
const instantiateSchemaClass = (ngPackageJson: NgPackageConfig) =>
  new NgPackageSchemaClass(ngPackageJson);
 ...
 */
const resolveUserPackage = async (
  folderPathOrFilePath: string
): Promise<UserPackage | undefined> => {
  const pathStats = await lstat(folderPathOrFilePath);
  const fullPath = path.isAbsolute(folderPathOrFilePath)
    ? folderPathOrFilePath
    : path.resolve(folderPathOrFilePath);
 ...
 */
const findSecondaryPackagesPaths = async (
...
const secondaryEntryPoint = (
  ...
  if (path.resolve(basePath) === path.resolve(primaryDirectoryPath)) {
    log.error(
      `Cannot read secondary entry point. It's already a primary entry point. path=${basePath}`
    );
  ...
  const secondaryModuleId = ensureUnixPath(
    `${primary.moduleId}/${relativeSourcePath}`
  );
...
export const discoverPackages = async ({
  ...
  const secondaries = await findSecondaryPackagesPaths(
      ...
      Promise.all(
        folderPaths.map(folderPath =>
          resolveUserPackage(folderPath).catch(() => {
            log.warn(
              `Cannot read secondary entry point at ${folderPath}. Skipping.`
            );
  ...
  if (secondaries.length > 0) {
    log.debug(
      `Found secondary entry points: ${secondaries
        .map(e => e.moduleId)
        .join(", ")}`
    );
  }
