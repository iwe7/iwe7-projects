...
} from "./entry-point/resources/template.di";
import {
  COMPILE_NGC_TOKEN,
  COMPILE_NGC_TRANSFORM
} from "./entry-point/ts/compile-ngc.di";
import {
  TRANSFORM_SOURCES_TOKEN,
  TRANSFORM_SOURCES_TRANSFORM
} from "./entry-point/ts/transform-sources.di";
import {
  WRITE_BUNDLES_TRANSFORM,
  WRITE_BUNDLES_TRANSFORM_TOKEN
} from "./entry-point/write-bundles.di";
import {
  WRITE_PACKAGE_TRANSFORM,
  WRITE_PACKAGE_TRANSFORM_TOKEN
} from "./entry-point/write-package.di";
import { entryPointTransformFactory } from "./entry-point.transform";

export const ENTRY_POINT_TRANSFORM_TOKEN = new InjectionToken<Transform>(
  `ng.v5.entryPointTransform`
);
