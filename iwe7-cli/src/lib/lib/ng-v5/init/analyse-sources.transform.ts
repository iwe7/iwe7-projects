...
export const analyseSourcesTransform: Transform = pipe(
  map(graph => {
    ...
    for (let entryPoint of entryPoints) {
      ...
      const extractDependencies = analyseDependencies(
        (sourceFile, moduleId) => {
          log.debug(`Found dependency in ${sourceFile.fileName}: ${moduleId}`);
          const dep = entryPoints.find(
            ep => ep.data.entryPoint.moduleId === moduleId
          );
          if (dep) {
            log.debug(
              `Found entry point dependency: ${
                entryPoint.data.entryPoint.moduleId
              } -> ${dep.data.entryPoint.moduleId}`
            );
      ...
      const tsSourcesNode = new TypeScriptSourceNode(
        tsUrl(entryPoint.data.entryPoint.moduleId)
      );
      tsSourcesNode.data = transformSourceFiles(entryPoint.data.tsConfig, [
        extractResources,
        extractDependencies
      ]);
