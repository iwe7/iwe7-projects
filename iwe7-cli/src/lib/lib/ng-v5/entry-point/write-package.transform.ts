...
export const writePackageTransform: Transform = transformFromPromise(
  async graph => {
    const entryPoint = graph.find(isEntryPointInProgress()) as EntryPointNode;
    const ngEntryPoint: NgEntryPoint = entryPoint.data.entryPoint;
    const ngPackage: NgPackage = graph.find(
      node => node.type === "application/ng-package"
    ).data;
    ...
    const declarationFiles = await globFiles(
      `${path.dirname(ngEntryPoint.entryFilePath)}/**/*.d.ts`,
      {
    ...
    await copyFiles(
      declarationFiles,
      path.dirname(destinationFiles.declarations)
    );
...
export async function writePackageJson(
  ...
  const whitelist = pkg.whitelistedNonPeerDependencies.map(
