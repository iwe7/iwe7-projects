...
export const templateTransform: Transform = transformFromPromise(
  async graph => {
    log.info(`Rendering Templates`);

    const entryPoint = graph.find(isEntryPointInProgress());
    const templateNodes = graph
      .from(entryPoint)
      .filter(node => node.type === "text/html" && node.state !== "done");

    // TOTO [].forEach(async fn)
    const promises = templateNodes.map(templateNode => {
      const templateFilePath = templateNode.url.substring(
        URL_PROTOCOL_FILE.length
      );
