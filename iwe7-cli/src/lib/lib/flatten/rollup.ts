...
import {
  ExternalModuleIdStrategy,
  DependencyList
} from "./external-module-id-strategy";
...
export async function rollupBundleFile(opts: RollupOptions): Promise<void[]> {
  log.debug(
    `rollup (v${rollup.VERSION}) ${opts.entry} to ${opts.dest} (${opts.format})`
  );

  const externalModuleIdStrategy = new ExternalModuleIdStrategy(
    opts.format,
    opts.dependencyList
  );
  ...
  const bundle = await rollup.rollup({
    context: "this",
    external: moduleId =>
      externalModuleIdStrategy.isExternalDependency(moduleId),
    input: opts.entry,
    plugins: [
      nodeResolve(),
      commonJs(),
  ...
  result.map.sources = result.map.sources.map(sourcePath => {
    ...
    if (sourcePath.indexOf(mapRootUrl) > 0) {
      return `${opts.sourceRoot}${sourcePath.substr(
        sourcePath.indexOf(mapRootUrl) + mapRootUrl.length
      )}`;
  ...
  return Promise.all([
    outputJson(sourcemapFullFile, result.map),
    outputFile(opts.dest, result.code)
  ]);
}
