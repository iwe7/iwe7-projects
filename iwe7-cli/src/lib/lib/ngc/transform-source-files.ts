...
function isTransformationResult<T extends ts.Node>(
  value: any
): value is ts.TransformationResult<T> {
  return (
    value.transformed instanceof Array && typeof value.dispose === "function"
  );
}
...
export function transformSourceFiles(
  ...
  if (isTransformationResult<ts.SourceFile>(source)) {
    ...
    const transformationResult: ts.TransformationResult<
      ts.SourceFile
      > = ts.transform(
