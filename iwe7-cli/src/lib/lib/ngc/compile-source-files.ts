...
export async function compileSourceFiles(
  ...
  const tsConfigOptions: ng.CompilerOptions = {
    ...tsConfig.options,
    ...extraOptions
  };

  // ts.CompilerHost
  let tsCompilerHost = createCompilerHostForSynthesizedSourceFiles(
    sourceFiles,
    tsConfigOptions
  );
  if (declarationDir) {
    tsCompilerHost = redirectWriteFileCompilerHost(
      tsCompilerHost,
  ...
  if (declarationDir) {
    const metadataBundleFile = flatModuleFile.replace(
      flatModuleFileExtension,
      ".metadata.json"
    );
    const metadataSrc = path.resolve(
      tsConfigOptions.declarationDir,
      metadataBundleFile
    );
