...
export class NgEntryPoint {
  ...
  public get destinationFiles(): DestinationFiles {
    ...
    if (this.secondaryData) {
      ...
      secondaryDir = path.relative(
        primaryDestPath,
        this.secondaryData.destinationPath
      );
    }

    const flatModuleFile = this.flatModuleFile;
    const pathJoinWithDest = (...paths: string[]) =>
      path.join(primaryDestPath, ...paths);

    return {
      metadata: pathJoinWithDest(
        secondaryDir,
        `${flatModuleFile}.metadata.json`
      ),
      declarations: pathJoinWithDest(secondaryDir, `${flatModuleFile}.d.ts`),
      esm2015: pathJoinWithDest(
        "esm2015",
        secondaryDir,
        `${flatModuleFile}.js`
      ),
    ...
    return includePaths.map(
      includePath =>
        path.isAbsolute(includePath)
          ? includePath
          : path.resolve(this.basePath, includePath)
    );
  }
